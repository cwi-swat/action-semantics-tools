...
# This is a simplified version of gen-asf-make.py (no deps on XT).
...
class ASFSDFModule:
    ...
    def parse_all_sdfs(self):
        f = os.popen('find . -name "*.sdf"')
        lines = f.readlines()
        f.close()
        for l in lines:
            m = string.rstrip(l)
            if not os.path.isfile(m + '.baf'):
                os.system("parse-sdf2 -v -i %s -o %s.baf" % (m,m))
    
    def get_import_graph(self):
        self.parse_all_sdfs()
        f = os.popen("imports -g -l -r ./meta.conf %s.sdf.baf" % (self.name))
        line = f.readline()
        f.close()
        term = str2aterm(line)
